...
return function(mod)
  ...
  do
    ...
    local BR = { 0.5, 1.0 }
    P:register("vp_stars", {
      label = "NIGHT STARS", levels = { "OFF", "DIM", "BRIGHT" }, priority = 9,
      available = haveCanvas,
      update = function(dt, level) bright = BR[level] or 0; t = t + (dt or 0) end,
      worldPresent = function(canvas, ctx)
        local hh = currentHour(mod)
        if canvas == nil or bright <= 0 or not (hh < 6 or hh >= 20) then return canvas end
        ...
        for i = 1, 60 do
          ...
          love.graphics.setColor(1, 1, 0.95, tw * bright); love.graphics.circle("fill", x, y, r)
        end
        love.graphics.pop()
        return o
      end,
  ...
  do
    local ensure = canvasCache()
    local mode, t = 0, 0
    P:register("vp_weather", {
      label = "WEATHER", levels = { "OFF", "RAIN", "SNOW", "FOG" }, priority = 8,
      available = haveCanvas,
